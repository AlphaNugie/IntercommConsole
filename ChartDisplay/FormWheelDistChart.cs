...
namespace ChartDisplay
{
    ...
    public class WheelDists
    {
        ...
        {
            return row == null ? null : new WheelDists() { WheelLeftDist = double.Parse(row["wheel_left_dist"].ToString()), WheelRightDist = double.Parse(row["wheel_right_dist"].ToString()), YawAngle = double.Parse(row["yaw_plc"].ToString()) / 10 + 16 };
        }
    }
}
