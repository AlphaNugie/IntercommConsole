        ...
        public override void LoopContent()
        {
            switch (Posture.Type)
            {
                ...
                case PostureType.PLC:
                    Posture.WalkingPosition = Const.OpcDatasource.WalkingPosition_Plc;
                ...
                case PostureType.AUTOMATIC:
                    ...
                    #region 行走位置校对
                    bool walking_diff_limited = Math.Abs(Const.OpcDatasource.WalkingPosition_Plc - Const.GnssInfo.WalkingPosition) <= Config.WalkingThreshold && Const.IsPlcPostureValid && Const.IsGnssValid; //编码器行走与北斗行走差值是否在阈值内（PLC，北斗数据同时可用）
                    Posture.WalkingPosition = !walking_diff_limited && Const.IsGnssValid ? Const.GnssInfo.WalkingPosition : Const.OpcDatasource.WalkingPosition_Plc; //只有当行走差值不在阈值内同时北斗数据可用的情况下采用北斗行走位置，否则均采用编码器行走位置
